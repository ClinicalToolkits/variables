...
import { CURLY_BRACE_ENCLOSURE, DataType } from "@clinicaltoolkits/type-definitions";
...
import { Variable, DBVariable, VariableIdToken, VariableContent } from "../types";
  ...
  bIncludeAutoGeneratedVariables = true,
    ...
    try {
      ...
      for (const dbVariable of data) {
        const variableContent = await fetchVariableContent(dbVariable.id, undefined, variableContentAffixParams, variableRegexRules);
      ...
      const finalData: Variable[] =
        await data?.reduce<Promise<Variable[]>>(
          async (accPromise, dbVariable: DBVariable) => {
            ...
            // Convert database rows to application model
            const convertedVariable = convertDBVariableToVariable(dbVariable, entityId, entityVersionId, labelPrefix, variableSetParams);
        ...
        for (const variable of finalData) {
          ...
          const descriptionText = variable ? getVariableDescriptionAsString(variable) : undefined
          console.log("variableDescriptionText: ", variable, descriptionText);
        }
