...
import { DataType, EntityId } from "@clinicaltoolkits/type-definitions";
import { convertDBVariableToVariableData, enrichSubvariableProperties, createAutoGeneratedVariables, applyDescriptiveRatings } from "./utility";
import { fetchDescriptiveRatingsArray, DescriptiveRating } from "../descriptive-ratings";
import { Variable, DBVariable, VariableIdToken, VariableContent, DEMOGRAPHICS, VariableFunctions, VariableData, wrapVariables } from "../types";
import { fetchVariableContent, getVariableAffixRules, getVariableDescriptionAsString, setVariableContent } from "../utility";
import { IAffixParams, contentBlockStore } from "@clinicaltoolkits/content-blocks";
...
export const fetchVariable = async ({ variableId, entityId, entityVersionId, labelPrefix }: FetchVariableParams): Promise<Variable | undefined> => {
  ...
  return wrapVariables(convertedVariableData);
};
...
export const fetchVariableBatch = async ({
  ...
  bIncludeAutoGeneratedVariables = true,
    ...
    try {
      ...
      const finalData: VariableData[] = await (data as unknown as DBVariable[])?.reduce<Promise<VariableData[]>>( // TODO: fix types - should not need 'as unknown as'
        async (accPromise, dbVariable: DBVariable) => {
          ...
          if (bIncludeAutoGeneratedVariables) {
            createAutoGeneratedVariables(convertedVariableData, acc, additionalAccPromises, variableSetDescriptiveRatings);
          }
      ...
      return wrapVariables(finalData);
    } catch (error) {
