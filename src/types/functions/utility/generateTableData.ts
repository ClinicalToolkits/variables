...
import { Variable, VariableMap, VariableFunctions } from "../..";
...
import { getVariableSubgroupsToDisplay } from "..";
...
interface FilterVariablesForTableInternalReturn {
  tableDataVariables: Variable[];
  tableFooterVariables: Variable[];
  tableMetadata: VariableTableMetadata;
}

export const filterVariablesForTableInternal = (variables: Variable[], bRemoveUnusedVariableRows = false): FilterVariablesForTableInternalReturn => {
  ...
  variables.forEach((variable) => {
    ...
    if (
      ...
      variableMetadata?.bIncludeInDynamicTable !== false && variable.isVisible() && !variable.getMetadata()?.bAutoGenerated //TODO: Monitor not including `bAutoGenerated` for errors. Seems to solve the issue of auto-generated variables showing up in their own rows.
    ) {
      tableDataVariables.push(variable);
    } else if (variable.isVisible() && variable.getMetadata()?.bIncludeInTableFooter === true) {
...
// TODO: Need to better differentiate this from filterVariablesForTableInternal
interface FilterVariablesForTableProps {
...
export const filterVariablesForTable = ({ inVariables, inVariableMap, bInRemoveUnusedVariableRows = false, bInUnifiedTable }: FilterVariablesForTableProps): FilterVariablesForTableReturn => {
  ...
  if (bInUnifiedTable) {
    const { tableDataVariables, tableFooterVariables, tableMetadata } = filterVariablesForTableInternal(inVariables,  bInRemoveUnusedVariableRows);
    tableDataBySubgroup['all'] = { tableDataVariables, tableMetadata };
    variablesForFooter.push(...tableFooterVariables);
  } else {
    const { subgroupsToDisplay } = getVariableSubgroupsToDisplay(inVariables);

    for (const [subgroupKey, variableKeys] of Object.entries(subgroupsToDisplay)) {
      const variables = VariableFunctions.getVariablesArray({inVariableMap, inVariableIds: variableKeys });
      const { tableDataVariables, tableFooterVariables, tableMetadata } = filterVariablesForTableInternal(variables, bInRemoveUnusedVariableRows);
