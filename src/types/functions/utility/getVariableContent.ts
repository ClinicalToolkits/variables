...
import { Variable, VariableContent, VariableData, VariableMap, createVariable } from "../..";
...
const log = logger.scope(SCOPES.types.functions.utility.get_variable_content);
...
export const fetchVariableContent = async (inDbVariableId: string, property?: string, inEntityId?: string, inEntityVersionId?: string): Promise<VariableContent | undefined> => {
  ...
  const affixParams: IAffixParams = {
    ...
    inEnclosure: contentBlockStore.getEnclosure(),
  };
  const regexRules: RegexRuleArray = getVariableAffixRules(affixParams);
  log.debug("fetchVariableContent - affixParams: ", affixParams);
  ...
  log.debug("fetchVariableContent - templateBlocks: ", templateBlocks);
...
export const setVariableDescriptionContentBlocks = (inVariable: Variable, inContentBlocks: ContentBlock[]): Variable => {
  ...
  return createVariable(variableData);
};
...
export const getContentBlocksFromVariableInterpretation = (inVariable: Variable, variableMap: VariableMap, bRemoveUnusedContentControls: boolean): ContentBlock[] | undefined => {
  ...
  const conditionalSubBlocks = getSubBlocksFromConditionalBlock({ conditionalBlock, objectMap: variableMap, bRemoveUnusedContentControls, shouldDisplayObjectContent: (variable: Variable) => variable.isVisible() }); // TODO: At some point should probably make the object that `shouldDisplayObjectContent` uses a generic type rather any and hardcoding it to Variable but that has cascaing effects.
