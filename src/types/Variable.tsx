...
import { defineModel, ModelInstance, RegexRuleArray } from "@clinicaltoolkits/utility-functions";
import { UUID, DataType, Tag, InfoFieldConfig, ComboboxData, ObjectInfoConfig, Age, emptyTag, asUUID, ID_SEPERATOR, EVisibility, convertEnumToComboboxDataArray, generateUUID, isHidden, isVisible, Nomen, getDisplayName, setImmutableAtPath, PathsToFields, PathValue } from "@clinicaltoolkits/type-definitions";
import { IAffixParams, ITemplateBlock, Editor } from "@clinicaltoolkits/content-blocks";
import { DBVariableMetadata, VariableMetadata, emptyVariableMetadata } from "./VariableMetadata";
import { renderVariableTooltipContent } from "./functions/child-variables/renderVariableTooltipContent";
import { getSubgroupNameForVariable } from "./functions/getVariableSubgroups";
...
export interface VariableData {
  ...
  categoryDisplayName?: Nomen | null; // TODO: Moving from subgroupTag to categoryDisplayName
...
export const variableDataKeys: (keyof VariableData)[] = [
  "id",
  "idToken",
  "fullName",
  "abbreviatedName",
...
export interface ExtraVariableMethods {
  isHidden(): boolean;
  isVisible(): boolean;
  ...
  /** Returns the category name for the variable, falling back to subgroupTag name if categoryDisplayName is not set (subgroupTag is the deprecated method of handling variable grouping). */
  getCategory(): string;
  getAssociatedCompositeVariableId(): string | undefined;
  withVisibility(inVisibility: EVisibility): Variable;
  /**
  * Update any property on VariableData (by dot-path) and return a NEW Variable.
  * Fully typed: `value` must match the path's value type.
  */ 
  withAtPath<P extends PathsToFields<VariableData>>(path: P, value: PathValue<VariableData, P>): Variable;
}

const VARIABLE_NAME = "Variable" as const;
const VARIABLE_MODE = "lax" as const;

export type Variable = ModelInstance<VariableData, typeof VARIABLE_MODE, ExtraVariableMethods, typeof VARIABLE_NAME>;
...
const VariableModel = defineModel<Variable>({
  name: VARIABLE_NAME,
  getters: {
  ...
  extras: {
    ...
    isAutoGenerated() {
      return this.getMetadata()?.bAutoGenerated ?? false;
    },
    getVariableId() {
    ...
    getCategory(): string {
      ...
      const out = categoryDisplayName ? getDisplayName(categoryDisplayName).plural.long : this.getSubgroupTag()?.displayName;
      return out || getSubgroupNameForVariable(this);
    },
    getAssociatedCompositeVariableId(): string | undefined {
      return this.getMetadata()?.associatedCompositeVariableIdToken?.variableId;
    },
    ...
    withVisibility(inVisibility) {
      return this.withMetadata({ ...this.getMetadata(), visibility: inVisibility });
    },
    withAtPath<P extends PathsToFields<VariableData>>(
      this: Variable,
      path: P,
      value: PathValue<VariableData, P>
    ): Variable {
      // Get raw data, set immutably, re-create a new model instance.
      const raw = this.toJSON(); // VariableData
      const next = setImmutableAtPath(raw, path as string, value) as VariableData;
      return VariableModel.create(next);
    }
  },
  immutable: true,
  allKeys: variableDataKeys,
  mode: VARIABLE_MODE,
});
...
export class VariableIdToken {
  ...
  get id(): string {
    return this.variableId;//`${this.entityId}${ID_SEPERATOR}${this.entityVersionId}${ID_SEPERATOR}${this.variableId}`;
  }
...
export interface DBVariable {
  ...
  variable_template_id?: string | null;
}
...
export type VariableGroupMap = Map<string, VariableGroup>;
